...
import Table from "layouts/senior/data/table";
import { tablehead } from "layouts/senior/data/head";  
...
function Seniors() {
    ...
    const handleSubmit = async (e) => {
        ...
        setReload(true);      
        try {
            const response = await axios.post(apiRoutes.seniorRetrieve + '?page=' + 1, formData, {headers});
  ...
  const fetchNextPrevTasks = (link) => {
    ...
    axios.post(apiRoutes.seniorRetrieve + '?page=' + nextPage, formData, {headers})
    .then(response => {
      setFetchdata(response.data.users);
      passToSuccessLogs(response.data, currentFileName);
      setReload(false);      
          ...
          <SoftBox p={2}>
            <SoftBox >   
              ...
              <Card className="px-md-4 px-2 pt-3 pb-md-3 pb-2">
                    ...
                    <Grid container spacing={1} py={1} pb={2}>  
                        <Grid item xs={12} lg={8} className="d-block d-md-flex">
                            ...
                            <select className="form-select-sm text-secondary rounded-5 me-2 cursor-pointer border span" name="grade" value={formData.grade} onChange={handleChange} >
                                ...
                                {seniorSelect && seniorSelect.map((senior) => (
                                <option key={senior.value} value={senior.value}>
                                        {senior.desc}
                                </option>
                                ))}
                            </select>
                            <select className="form-select-sm text-secondary rounded-5 me-2 cursor-pointer border span" name="track" value={formData.track} onChange={handleChange} >
                ...
                <SoftBox className="shadow-none table-container px-md-1 px-3 bg-gray rounded-5 bg-gray rounded-5" height={tableHeight} minHeight={50}>
