...
import List from "layouts/elections/components/ArchiveContainer/List";
import LeaderboardTwoToneIcon from '@mui/icons-material/LeaderboardTwoTone';

function ArchiveContainer({authUser, FROM, INFO, HandleRendering}) {
  const currentFileName = "layouts/elections/components/ArchiveContainer/index.js";
  ...
  useEffect(() => {
    if (reload) {
      setReload(true);
      axios.get(apiRoutes.archiveResult, { params: { info }, headers })
        .then(response => {
  ...
  return (
          ...
          <Grid item xs={12} md={5} sx={{ ml: "auto" }}>
            <AppBar position="static">
              ...
              >
                ...
                <Tab label="Final Results" onClick={setApply} icon={<LeaderboardTwoToneIcon />} />
              </Tabs> 
            </AppBar>
          </Grid>
        </Grid>
    ...
    {menu === "apply" && FROM === "ongoing" &&
      <List reload={reload} RESULT={fetchresult}
